...
const { tsFormatter, companyOverviewFormatter, balanceSheetFormatter } = require("./externalDataFormatter");


exports.externalDataFetcher = (ticker) => {
  const API_KEY = "process.env.API_KEY";
  const tsDailyUrl = `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=${ticker}&apikey=${API_KEY}`;
  const tsWeeklyUrl = `https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY_ADJUSTED&symbol=${ticker}&apikey=${API_KEY}`;
  const tsMonthlyUrl = `https://www.alphavantage.co/query?function=TIME_SERIES_MONTHLY_ADJUSTED&symbol=${ticker}&apikey=${API_KEY}`;
